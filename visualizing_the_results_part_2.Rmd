...
The first graph shows you a simple scatterplot of this measur of inequality index vs. the POLS209 democracy score. (Dots are "jittered" a bit to avoid overplotting). 
...
As we can see, there's not much correlation between levels of democracy and levels of inequality; though democracies are very slightly more democratic than non-democracies (as indicated by the slope of the line), the difference is so small as to be basically statistical noise.

It's possible that this is because of quirks in our measure of democracy. Maybe we don't have enough years, or you were too idiosyncratic in your judgments. But here's a scatterplot using the "Gold Standard" of democrayc measures, the [V-Dem polyarchy index](https://www.v-dem.net/en/):
...
There is a clearer relationship here, but it's pretty slight; lots of democracies are very unequal, and at least some non-democracies are fairly equal. Here's an interactive version of the same graph (hover over each dot to see what country-year it represents):

```{r vdemcorr_interactive}
library(plotly)

p <- ggplot(data = data, aes(x = v2x_polyarchy,
                             y = mean_value,
...
Basically, the most equal countries (in terms of net income) appear to be either Scandinavian social democracies or communist authoritarian regimes.

But perhaps that's not quite the right way of looking at things. Inequality may be affected by all sorts of things, and it takes time . It's possible that what we need to see is simply whether countries that become more democratic also become more equal. Here's how that looks with the POLS209 dataset, country by country:
...
We could complicate this figure, but basically, inequality seems pretty stable: becoming more or less democratic does not seem to change the level of inequality much anywhere. 

You may wonder whether this is perhaps an artifact of our measure, so let's try re-doing the graph using the V-Dem polyarchy index:
...
Again, not much change in inequality with changes in democracy levels is detectable here; democratization (or loss of democracy appears not to affect the level of democracy that much). What is going on here? Are these the wrong measures? 
...
Basically, in Albania and Russia income inequality increased greatly with the transition from communist authoritarianism (more democracy meant more inequality); at the bottom, in Bhutan, more democracy meant less inequality.
...
I'll leave things here for now, but more analysis coming next week!
